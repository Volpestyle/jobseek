...
export class S3Service {
  ...
  async getUploadUrl(userId: string, filename: string, contentType: string): Promise<{ uploadUrl: string; s3Key: string }> {
  ...
  async listUserResumes(userId: string): Promise<{ key: string; size: number; lastModified: Date }[]> {
  ...
  async getPublicUrl(s3Key: string, expiresInDays: number = 7): Promise<string> {
    const command = new GetObjectCommand({
      Bucket: BUCKET_NAME,
      Key: s3Key,
    })
