...
export function useProfile(): UseProfileReturn {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      const storage = await storageService.getStorageForUser(session?.user?.id || null)
    ...
    try {
      ...
      if (profile) {
        ...
        updatedProfile = await storage.saveUserProfile({
          ...
          firstName: updates.firstName || session?.user?.name?.split(' ')[0] || '',
          lastName: updates.lastName || session?.user?.name?.split(' ').slice(1).join(' ') || '',
